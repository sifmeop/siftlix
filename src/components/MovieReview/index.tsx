...
const MovieReview: NextPage<IProps> = ({ movie }) => {
  ...
  const { t: tReviews } = useTranslation('global')
  ...
  const handleSubmit = async () => {
    ...
    try {
      await set(reference, { ...movie, ...values }).then(() => {
        message.success(
          `${tReviews('added-review')} for ${movie.title ? tGlobal('movie') : tGlobal('tv')} ${movie.title}`
        )
        setValue('')
  ...
  const removeReview = async () => {
    ...
    try {
      await remove(reference).then(() => {
        message.success(
          `${tReviews('removed-review')} ${movie.title ? tGlobal('movie') : tGlobal('tv')} ${movie.title}`
        )
      })
    } catch (e) {
  ...
  return (
    <>
      {reviews.some((item) => item.id === review.id) ? (
        <Tooltip title={tReviews('delete-review')}>
          <div>
            <TbPencilOff onClick={removeReview} className={styles.icon} size='1.5rem' />
          </div>
        </Tooltip>
      ) : (
        <Tooltip title={tReviews('rate-review')}>
          <div>
      ...
      <ModalView
        ...
        <div className='text-center'>
          <h1 className='font-medium text-xl mb-3'>{tReviews('review')}</h1>
          ...
          <p className='text-left text-lg mb-1'>{tReviews('your-review')}</p>
          <TextArea
            value={value}
            onChange={(e) => setValue(e.target.value)}
            autoSize
            className='mb-3'
            placeholder={tReviews('write-review')}
          />
          <p className='text-left text-lg mb-1'>{tReviews('your-rating')}</p>
